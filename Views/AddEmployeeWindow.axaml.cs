...
{
    public AddEmployeeWindow(MainWindowViewModel model)
    {
        InitializeComponent();
        DataContext = new AddEmployeeWindowViewModel(model);
    }

    private async void ChangePhoto(object sender, RoutedEventArgs e)
    {
        var button = (Button)sender;
        var context = (AddEmployeeWindowViewModel)button.DataContext;
        ...
        if (result != null && result.Length > 0)
        {
            using (FileStream fs = File.OpenRead(result[0]))
            {
                ...
                {
                    bp.Save(ms);
                    context!.Employee.Photo = ms.ToArray();
                    context!.Employee.AvatarBitmap = bp;
                }
            }
        }
    }
